...
Namespace SpreadsheetControl_WPF_Examples
	Public NotInheritable Class RowAndColumnActions
		#Region "Actions"
        Public Shared FreezeRowAction As Action(Of SpreadsheetControl) = AddressOf FreezeRow
        Public Shared FreezeColumnAction As Action(Of SpreadsheetControl) = AddressOf FreezeColumn
        Public Shared FreezePanesAction As Action(Of SpreadsheetControl) = AddressOf FreezePanes
        Public Shared UnfreezePanesAction As Action(Of SpreadsheetControl) = AddressOf UnfreezePanes
		#End Region

		#Region "#FreezeRow"
		Private Sub New()
		End Sub
		Private Shared Sub FreezeRow(ByVal control As SpreadsheetControl)
			control.BeginUpdate()
			Try
		...
		#Region "#FreezeColumn"
		Private Shared Sub FreezeColumn(ByVal control As SpreadsheetControl)
			control.BeginUpdate()
			Try
				'Access the active worksheet.
				Dim worksheet As Worksheet = control.Document.Worksheets.ActiveWorksheet
		...
		#Region "#FreezePanes"
		Private Shared Sub FreezePanes(ByVal control As SpreadsheetControl)
			'Access the active worksheet.
			Dim worksheet As Worksheet = control.Document.Worksheets.ActiveWorksheet
		...
		Private Shared Sub UnfreezePanes(ByVal control As SpreadsheetControl)
			control.BeginUpdate()
			Try
				'Access the active worksheet.
				Dim worksheet As Worksheet = control.Document.Worksheets.ActiveWorksheet
